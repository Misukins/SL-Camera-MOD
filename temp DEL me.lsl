...
dataserver(key id, string data)
{
    ...
    if(i != -1)
    {
        ...
        // debugOutput("dataserver: Returning online status");
        llOwnerSay("<onlineStatus>" + (string)llList2String(onlineavis,i)+"|"+data);
        // Remove the avatar we just reported from the list.
        onlinereqs = llDeleteSubList(onlinereqs,i-1,i);
        onlineavis = llDeleteSubList(onlineavis,i-1,i);
    }// End If
} //End dataserver
http_request(key httpReqID, string Method, string Body)
{
    ...
    else if (Method == "GET" || Method == "POST")
    {
        ...
        else if (cmd == "online_status")
        {
            ...
            key avi_id = (key)llList2String(commandList,1);
            integer id = (integer)llList2String(commandList,2);
            onlineavis += [id, avi_id];
            onlinereqs += [id, llRequestAgentData(avi_id, DATA_ONLINE)];
            //debugOutput("receive: Processing online request");
        }// End If
